      ...
      <div style={{display:'flex'}}>
        {[...Array(5).keys()].map((index) => (
          <StarIcon
            ...
            className={clsx(
            'reviewsStarrating',
              rating > index ? 'reviewsStarratingOpt' : 'reviewsStarratingOpt1',
            )}
          />
        ))}
      </div>
...
function Review({title, body, author, rating, className, ...props}){
    ...
    return(
        <figure 
        className={clsx("reviewfigure",className)}
        style={{animationDelay}}
        {...props}
        >
        <blockquote style={{color:'rgb(23 23 23 / 1)'}}>
            <StarRating rating={rating}/>
            <p className="reviewp">
                {title}
            </p>
            <p style={{marginTop:'0.75rem',fontSize:'1rem',lineHeight:'1.75rem'}}>
                {body}
            </p>
        </blockquote>
        <figcaption className="reviewfigurecaption">
            {author}
        </figcaption>
        </figure>
    )
...
return (
    <div
    ref={columnRef}
    className={clsx("reviewcolumndiv",className)}
    ...
    { isInView && (
      <>
        ...
        reviewClassName={(reviewIndex)=>clsx(
          reviewIndex >= column1.length + column3[0].length && "reviewOpt",
          reviewIndex >= column1.length && 'reviewcolumnOpt1'
        )}
        msPerPixel={10}/>

        <ReviewColumn
        reviews={[...column2,...column3[1]]}
        className='reviewcolumn1'
        reviewClassName={(reviewIndex)=>
          reviewIndex >= column2.length? 'reviewcolumn1Opt':''
        }
        msPerPixel={15}
        />
        ...
        className='reviewcolumn2'
        msPerPixel={10}/>
      </>
    )}
    <div className="reviewgriddiv1" />
    <div className="reviewgriddiv2"/>
    </div>
  )
}
