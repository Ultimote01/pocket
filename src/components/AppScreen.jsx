...
export function AppScreen({children,className, ...props}){
    ...
    return (
        <div className={className} {...props} style={{display:'flex',flexDirection:'column',...props?.style}}>
            <div style={{display:'flex',justifyContent:'space-between', padding:'1rem 1rem 0px 1rem'}}>
            <MenuIcon style={{height:'1.5rem',width:'1.5rem',flex:'none'}}/>
            <Logo style={{height:'1.5rem', flex:'none'}}/>
            <UserIcon style={{height:'1.5rem',width:'1.5rem',flex:'none'}}/>
            </div>
            {children}
        </div>
    )
...
AppScreen.Header = forwardRef(function AppScreenHeader({ children },ref){
    return(
        <div ref={ref} style={{marginTop:'1.5rem', padding:'0 1rem', color:'rgb(255 255 255 / 1)'}}>
        {children}
        </div>
    )
}
...
AppScreen.Title= forwardRef( function AppScreenTitle({ children }, ref){
    return (
        <div ref={ref} style={{fontSize:'1.5rem',lineHeight:'2rem', color:'rgb(255 255 255 /  1)'}}>
        {children}
        </div>
    )
}
...
AppScreen.Subtitle=forwardRef( function AppScreenSubtitle({ children }, ref){
 return (
    <div ref={ref} style={{fontSize:'0.875rem', lineHeight:'1.5rem',color:'rgb(115 115 115 / 1)'}}>
    {children}
    </div>
 )
}
...
AppScreen.Body = forwardRef( function AppScreenBody({ children, className }, ref){
    return (
        <div ref={ref} className={className} style={{marginTop:'1.5rem',flex:'1 1 auto', borderTopLeftRadius:'1rem',borderTopRightRadius:'1rem',backgroundColor:'rgb(255 255 255 / 1)'}}>
        {children}
        </div>
    )
}
